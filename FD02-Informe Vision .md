**![C:\\Users\\EPIS\\Documents\\upt.png][image1]**

**UNIVERSIDAD PRIVADA DE TACNA**

**FACULTAD DE INGENIERÍA**

**Escuela Profesional de Ingeniería de Sistemas**

 **Proyecto *Sistema Verificador de Cumplimiento OWASP***

Curso: *Calidad y Pruebas de Software*

Docente: *Patrick Jose Cuadros Quiroga*

**Integrantes:**

***Andia Navarro , Diego Fabrizio \- 2022073906***  
***Concha Llaca Gerardo Alejandro \- 2017057849***

**Tacna – Perú**  
***2026***

| CONTROL DE VERSIONES |  |  |  |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- |
| Versión | Hecha por | Revisada por | Aprobada por | Fecha | Motivo |
| 1.0 | MPV | ELV | ARV | 10/10/2020 | Versión Original |

# 

# 

# 

# 

# 

# 

# 

# 

# 

# 

# 

# **Sistema *Verificador de Cumplimiento OWASP***

# 

# **Informe de Factibilidad**

# 

# **Versión *{1.0}***

# 

| CONTROL DE VERSIONES |  |  |  |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- |
| Versión | Hecha por | Revisada por | Aprobada por | Fecha | Motivo |
| 1.0 | MPV | ELV | ARV | 10/10/2020 | Versión Original |

**ÍNDICE GENERAL**

1\.	Introducción	1

1.1	Propósito	1

1.2	Alcance	1

1.3	Definiciones, Siglas y Abreviaturas	1

1.4	Referencias	1

1.5	Visión General	1

2\.	Posicionamiento	1

2.1	Oportunidad de negocio	1

2.2	Definición del problema	2

3\.	Descripción de los interesados y usuarios	3

3.1	Resumen de los interesados	3

3.2	Resumen de los usuarios	3

3.3	Entorno de usuario	4

3.4	Perfiles de los interesados	4

3.5	Perfiles de los Usuarios	4

3.6	Necesidades de los interesados y usuarios	6

4\.	Vista General del Producto	7

4.1	Perspectiva del producto	7

4.2	Resumen de capacidades	8

4.3	Suposiciones y dependencias	8

4.4	Costos y precios	9

4.5	Licenciamiento e instalación	9

5\.	Características del producto	9

6\.	Restricciones	10

7\.	Rangos de calidad	10

8\.	Precedencia y Prioridad	10

9\.	Otros requerimientos del producto	10

	[b) Estándares legales](#heading=h.puz6751lav5f)	32

	[c) Estandares de comunicación](#heading=h.puz6751lav5f)	37

	[d) Estándares de cumplimiento de la plataforma](#heading=h.puz6751lav5f)	42

	[e) Estándares de calidad y seguridad](#heading=h.puz6751lav5f)	42

[CONCLUSIONES](#heading=h.b2aheha60rez)	46

[RECOMENDACIONES](#heading=h.8mtf5ucbcapi)	46

[BIBLIOGRAFÍA](#heading=h.yf2s3awso4zp)	46

1\.	Introducción	

1.1	Propósito

El propósito del presente documento es definir la visión general del sistema “Verificador de Cumplimiento OWASP”, describiendo su alcance, objetivos, características principales y el valor que aportará a los usuarios.

Este documento está dirigido a estudiantes, desarrolladores, docentes y organizaciones interesadas en mejorar la seguridad de sus aplicaciones mediante la identificación de vulnerabilidades basadas en estándares OWASP.

1.2	Alcance

El sistema permitirá analizar aplicaciones web y APIs con el objetivo de identificar vulnerabilidades de seguridad basadas en OWASP Top 10\.

El alcance incluye:

* Evaluación automática de seguridad  
* Generación de reportes detallados  
* Recomendaciones de mitigación  
* Interfaz web amigable

No incluye:

* Corrección automática de vulnerabilidades  
* Auditorías manuales avanzadas

	

1.3	Definiciones, Siglas y Abreviaturas

**OWASP:** Open Web Application Security Project

**XSS:** Cross-Site Scripting

**SQL Injection:** Inyección SQL

**API:** Interfaz de Programación de Aplicaciones

**Vulnerabilidad:** Debilidad que puede ser explotada

**Pentesting:** Pruebas de penetración

	

1.4	Referencias

OWASP Top 10

Documentación de OWASP ZAP

Normas de seguridad informática

	

1.5	Visión General

El documento presenta la visión del sistema, identificando problemas actuales, oportunidades de mejora, usuarios involucrados, funcionalidades principales y restricciones del sistema.	

2\.	Posicionamiento	

2.1	Oportunidad de negocio

Actualmente existe una creciente necesidad de herramientas que permitan evaluar la seguridad de aplicaciones web debido al incremento de ataques cibernéticos.

El sistema se posiciona como una solución accesible para:

* Instituciones educativas  
* Pequeñas empresas  
* Desarrolladores independientes

Ofrece una alternativa económica frente a herramientas comerciales costosas.

	

2.2	Definición del problema

Muchas aplicaciones presentan vulnerabilidades debido a:

* Falta de conocimiento en seguridad  
* Ausencia de herramientas automatizadas  
* Limitaciones económicas

Esto genera riesgos como:

* Pérdida de datos  
* Accesos no autorizados  
* Fallas en sistemas

El sistema busca resolver este problema mediante evaluaciones automáticas basadas en OWASP.

	

3\.	Descripción de los interesados y usuarios	

3.1	Resumen de los interesados

| Interesado | Descripción |
| :---- | :---- |
| Estudiantes | Usuarios académicos que desarrollan software |
| Docentes | Supervisan proyectos |
| Empresas | Requieren seguridad en sus sistemas |
| Área de TI | Gestiona seguridad |

	

3.2	Resumen de los usuarios

| Usuario | Descripción |
| :---- | :---- |
| Desarrollador | Evalúa su aplicación |
| Administrador | Gestiona el sistema |
| Auditor | Analiza reportes |

	

3.3	Entorno de usuario

El sistema será utilizado en:

* Laboratorios académicos  
* Oficinas de desarrollo  
* Entornos remotos vía internet

Requiere conexión a internet y navegador web.

	

3.4	Perfiles de los interesados

* Conocimientos básicos en tecnología  
* Interés en seguridad informática  
* Necesidad de evaluar software

	

3.5	Perfiles de los Usuarios

* Nivel técnico medio  
* Manejo de aplicaciones web  
* Conocimiento básico de seguridad

	

3.6	Necesidades de los interesados y usuarios

* Detectar vulnerabilidades rápidamente  
* Obtener reportes claros  
* Mejorar seguridad del software  
* Reducir riesgos

	

4\.	Vista General del Producto	

4.1	Perspectiva del producto

El sistema será una aplicación web independiente que puede integrarse con herramientas de análisis como OWASP ZAP.

	

4.2	Resumen de capacidades

* Escaneo de vulnerabilidades  
* Generación de reportes  
* Panel de control  
* Recomendaciones de seguridad

	

4.3	Suposiciones y dependencias

* Acceso a internet  
* Uso de herramientas externas (OWASP ZAP)  
* Disponibilidad de servidores

	

4.4	Costos y precios

El sistema tendrá un costo de desarrollo aproximado de **S/ 11,890**, y podrá ofrecerse como herramienta gratuita o de bajo costo en el ámbito académico.

	

4.5	Licenciamiento e instalación

* Software basado en herramientas open source  
* Instalación en servidor local o nube  
* Acceso mediante navegador web

5\.	Características del producto

* Evaluación de seguridad OWASP Top 10  
* Generación de reportes detallados  
* Interfaz amigable  
* Panel de administración  
* Historial de análisis

	

6\.	Restricciones

* Limitaciones de hardware  
* Dependencia de herramientas externas  
* Tiempo de desarrollo  
* Nivel de conocimiento del equipo

	

7\.	Rangos de calidad

* Alta disponibilidad  
* Seguridad de datos  
* Precisión en resultados  
* Usabilidad

	

8\.	Precedencia y Prioridad

Alta prioridad:

* Módulo de análisis  
* Generación de reportes

Media prioridad:

* Interfaz gráfica  
* Panel administrativo

	

9\.	Otros requerimientos del producto	

	[b) Estándares legales](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.puz6751lav5f)

* Protección de datos  
* Uso de software libre

	

	[c) Estándares de comunicación](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.puz6751lav5f)

* HTTP/HTTPS  
* APIs REST

	d) Estándares de cumplimiento de la plataforma

* Compatibilidad web  
* Navegadores modernos

	

	[e) Estándares de calidad y seguridad](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.puz6751lav5f)

* OWASP Top 10  
* Buenas prácticas de desarrollo seguro

	

[CONCLUSIONES](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.b2aheha60rez)

El desarrollo del sistema “Verificador de Cumplimiento OWASP” representa una solución innovadora y necesaria frente a los problemas actuales de seguridad en aplicaciones web, tanto en el ámbito académico como empresarial. A lo largo del presente documento de visión, se ha identificado claramente la necesidad de contar con herramientas automatizadas que permitan evaluar vulnerabilidades de manera eficiente, accesible y confiable.

	

[RECOMENDACIONES](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.8mtf5ucbcapi)

Se recomienda llevar a cabo el desarrollo del sistema “Verificador de Cumplimiento OWASP”, considerando las siguientes acciones para asegurar su éxito:

* Implementar el sistema utilizando herramientas actualizadas de seguridad como OWASP ZAP, garantizando análisis más precisos y confiables.  
* Capacitar a los desarrolladores y usuarios finales en temas básicos de ciberseguridad y estándares OWASP, con el fin de mejorar la interpretación de los resultados obtenidos.  
* Realizar pruebas continuas durante el desarrollo para validar el correcto funcionamiento del sistema y reducir errores.  
* Actualizar periódicamente el sistema conforme a las nuevas versiones del OWASP Top 10, asegurando su vigencia frente a nuevas amenazas.  
* Integrar el sistema con otros entornos de desarrollo o plataformas educativas para ampliar su uso y alcance.  
* Implementar medidas adicionales de seguridad dentro del propio sistema, como autenticación segura y protección de datos.  
* Documentar adecuadamente el sistema para facilitar su mantenimiento y futuras mejoras.  
* Considerar la escalabilidad del sistema, permitiendo su uso en entornos empresariales en el futuro.

	

[BIBLIOGRAFÍA](https://docs.google.com/document/d/1vCyhruMyFHhHw1jOeTqEGTogJ4Y5Sib5IUSa9aVPKWc/edit?pli=1#heading=h.yf2s3awso4zp)

- *OWASP foundation, the Open Source Foundation for Application Security*. (s/f). Owasp.org. Recuperado el 7 de abril de 2026, de [https://owasp.org/](https://owasp.org/) 


	

[image1]: <data:image/png;base64,>